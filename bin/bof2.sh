...
python "$TOOLS_PATH/bof2.py" insert_text -s "$SOURCE" -d "$DESTINATION" -t2 "$TABLE2" -tp "$TRANSLATED_TEXT_PATH" -db "$DB" -u "$USER"
python "$TOOLS_PATH/bof2.py" insert_gfx -d "$DESTINATION" -tp "$TRANSLATED_GFX_PATH"
# python "$TOOLS_PATH/bof2.py" insert_misc -d "$DESTINATION" -t1 "$TABLE1" -t2 "$TABLE3" -tp "$TRANSLATED_MISC_PATH"
