...
python "$TOOLS_PATH/som.py" insert_tilemap -d "$TRANSLATED_MISC_PATH/intro-data.bin" -tp "$TRANSLATED_MISC_PATH"
...
python "$TOOLS_PATH/som.py" insert_text -s "$SOURCE" -d "$DESTINATION" -t1 "$TABLE4" -t2 "$TABLE6" -tp "$TRANSLATED_TEXT_PATH" -db "$DB" -u "$USER"
python "$TOOLS_PATH/som.py" insert_misc -d "$DESTINATION" -t1 "$TABLE5" -tp "$TRANSLATED_MISC_PATH"
...
# python ../mte_optimizer.py table -s "$RESOURCE_PATH/translated_text/dump_events_clomax.txt" -d "dict.txt" -c "clean.txt" -b 1 -m 2 -M 2 -l 59 -o 128 --game som
