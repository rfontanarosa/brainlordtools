...
DESTINATION="/Users/rfontanarosa/git/Final-Fantasy-Mystic-Quest-ITA/ffmq_new.sfc"
...
python3 ../brainlordtools/ffmq.py dump_misc -s "$SOURCE" -t1 "$TABLE1" -dp "$DUMP_MISC_PATH"
python3 ../brainlordtools/ffmq.py insert_misc -s "$SOURCE" -d "$DESTINATION" -t1 "$TABLE1" -t2 "$TABLE1" -tp "$TRANSLATION_MISC_PATH"
